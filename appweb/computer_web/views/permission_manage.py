...
def get_role_pri():  # 获取角色权限
    ...
    if param_judge(params, ["role_id", "parent_id"]):
        quary_sql = """SELECT privilege_code as pri_code, name as pri_name, ord_num, leaf_flag, icon, parent_code, target, remarks, url, pri_type FROM usr_grant, usr_privilege WHERE privilege_code=usr_privilege.code and usr_grant.role_id="{}";""".format(params.get("role_id"))

        res = MysqlHelper.fetchall(sql=quary_sql)
        if res == -2:
            res = []
        if params.get("parent_id"):
            quary_sql_parent = """SELECT privilege_code as pri_code, name as pri_name, ord_num, leaf_flag, icon, parent_code, target, remarks, url, pri_type FROM usr_grant, usr_privilege WHERE role_id="{}" and privilege_code=usr_privilege.code;""".format(params.get("parent_id"))
...
def modify_role_pri():  # 分配角色权限
    ...
    if param_judge(params, ["pri_codes", "role_id"]):
        ...
        if len(pri_name_code_list) == len(list(set(pri_name_code_list).intersection(set(parent_privilege_all)))):
            ...
            # 第五步: 添加前端提交的权限
            insert_role_pri = """INSERT INTO yilu_park.usr_grant(id, role_id, privilege_code, del_flag, create_time) VALUES """
...
def add_pri():  # 添加权限
    ...
    if param_judge(params,
        ...
        # 判断权限名称是否同名
        judge_name = MysqlHelper.fetchone(sql="SELECT name, code FROM yilu_park.usr_privilege WHERE name=%s;", params=(params.get("name"),))
        if judge_name:
            return get_result(success=False, message="权限名称已存在", error_code=PRI_ERROR)

        # 判断父权限id是否存在
        if params.get("parent_code"):
            judge_parent_id = MysqlHelper.fetchone(sql="SELECT name, code FROM yilu_park.usr_privilege WHERE code=%s;", params=(params.get("parent_code"),))
            ...
            find_code = MysqlHelper.fetchall(
                sql="""SELECT code FROM yilu_park.usr_privilege WHERE code LIKE "{}%";""".format(params.get("parent_code")))
        ...
        insert_sql = MysqlHelper.insert(
            ...
            params=(random_string(), code, params.get("name"), params.get("pri_type"), code, params.get("name"),
...
def del_pri():  # 删除权限
    ...
    if param_judge(params, ["role_id", "pri_code"]):
        sql1 = """DELETE FROM yilu_park.usr_grant WHERE privilege_code="{}" and role_id = "{}";""".format(params.get("pri_code"), params.get("role_id"))
